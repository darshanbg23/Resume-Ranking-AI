...
import {
  CheckCircle, Schedule, HourglassEmpty, Cancel,
  EventAvailable, Verified, FiberManualRecord
} from '@mui/icons-material';
import type { ApplicationStatus, JobStatus, InterviewStatus } from '../types';

type BadgeVariant = ApplicationStatus | JobStatus | InterviewStatus | 'processing' | 'pending' | 'processed' | 'failed' | 'excellent' | 'good' | 'average' | 'poor';
...
export const StatusBadge: React.FC<StatusBadgeProps> = ({ status, size = 'md', showIcon = true }) => {
  ...
  return (
    <span className={`badge ${cfg.className} ${sizeClass}`}>
      {showIcon && <Icon style={{ fontSize: size === 'sm' ? 10 : 12 }} />}
      {cfg.label}
    </span>
  );
};
